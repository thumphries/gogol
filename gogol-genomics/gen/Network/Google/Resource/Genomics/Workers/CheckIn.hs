...
wciPp :: Lens' WorkersCheckIn Bool
wciPp = lens _wciPp (\ s a -> s{_wciPp = a})
...
-- | OAuth bearer token.
wciBearerToken :: Lens' WorkersCheckIn (Maybe Text)
wciBearerToken
  = lens _wciBearerToken
      (\ s a -> s{_wciBearerToken = a})
