...
ptscsPp :: Lens' ProjectsTracesSpansCreateSpan Bool
ptscsPp = lens _ptscsPp (\ s a -> s{_ptscsPp = a})
...
-- | OAuth bearer token.
ptscsBearerToken :: Lens' ProjectsTracesSpansCreateSpan (Maybe Text)
ptscsBearerToken
  = lens _ptscsBearerToken
      (\ s a -> s{_ptscsBearerToken = a})
