...
-- users.
--
-- /See:/ <https://developers.google.com/apps-script/api/ Google Apps Script API Reference> for @script.projects.getMetrics@.
module Network.Google.Resource.Script.Projects.GetMetrics
    (
    -- * REST Resource
...
pgmPp :: Lens' ProjectsGetMetrics Bool
pgmPp = lens _pgmPp (\ s a -> s{_pgmPp = a})
...
-- | OAuth bearer token.
pgmBearerToken :: Lens' ProjectsGetMetrics (Maybe Text)
pgmBearerToken
  = lens _pgmBearerToken
      (\ s a -> s{_pgmBearerToken = a})
