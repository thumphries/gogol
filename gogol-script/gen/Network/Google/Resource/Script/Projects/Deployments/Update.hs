...
--
-- /See:/ <https://developers.google.com/apps-script/api/ Google Apps Script API Reference> for @script.projects.deployments.update@.
module Network.Google.Resource.Script.Projects.Deployments.Update
    (
    -- * REST Resource
...
pduPp :: Lens' ProjectsDeploymentsUpdate Bool
pduPp = lens _pduPp (\ s a -> s{_pduPp = a})
...
-- | OAuth bearer token.
pduBearerToken :: Lens' ProjectsDeploymentsUpdate (Maybe Text)
pduBearerToken
  = lens _pduBearerToken
      (\ s a -> s{_pduBearerToken = a})
