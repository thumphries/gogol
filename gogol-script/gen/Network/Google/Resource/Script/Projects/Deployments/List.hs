...
--
-- /See:/ <https://developers.google.com/apps-script/api/ Google Apps Script API Reference> for @script.projects.deployments.list@.
module Network.Google.Resource.Script.Projects.Deployments.List
    (
    -- * REST Resource
...
pdlPp :: Lens' ProjectsDeploymentsList Bool
pdlPp = lens _pdlPp (\ s a -> s{_pdlPp = a})
...
-- | OAuth bearer token.
pdlBearerToken :: Lens' ProjectsDeploymentsList (Maybe Text)
pdlBearerToken
  = lens _pdlBearerToken
      (\ s a -> s{_pdlBearerToken = a})
